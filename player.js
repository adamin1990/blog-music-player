/**
 * musicPlayer
 * 修改自网络 https://www.lixiaopeng.top
 */
...
var adaminPlayerInit = function () {
    var isPhone = false;
    // Test环境
    var isTest = typeof AdaminPlayerTest !== "undefined" && AdaminPlayerTest;
    var testKey = typeof AdaminPlayerTestKey !== "undefined" ? AdaminPlayerTestKey : null;
    var styleLoaded = typeof AdaminPlayerStyleLoaded !== "undefined" && AdaminPlayerStyleLoaded;
    ...
    if(!styleLoaded){
        $("head").append('<link rel="stylesheet" type="text/css" href="https://www.lixiaopeng.top/Public/static/css/player.css">');
        $("head").append('<link href="https://libs.baidu.com/fontawesome/4.2.0/css/font-awesome.css" rel="stylesheet" type="text/css">');
        AdaminPlayerStyleLoaded = true;
    }

    $("body").append('<div id="adaminPlayer">\n' +
        '    <div class="player">\n' +
        ...
        '    </div>\n' +
        '</div>\n' +
        '<div id="adaminTips"></div>\n' +
        '<div id="adaminLrc"></div>');
    ...
    var audio = new Audio(),
        ...
        ycgeci = true,
        first = 1,
        volume = $.cookie('adamin_player_volume') ? $.cookie('adamin_player_volume') : '0.666',
        albumId = 0,
        songId = 0,
        songTotal = 0,
        random = true,
    ...
    if(isPhone){$('#adaminLrc').addClass('phone');$player.addClass('phone');$(".new-volume",$player).hide();}
    ...
    $cover.html('<img src="">');
    $songName.html('<a style="color:#f00">initing...</a>');
    $songFrom.html('<a style="color:#f00">www.lixiaopeng.top</a>');
    $songFrom1.html('<a style="color:#f00">Music Player</a>');
    $songFrom3.html('<i class="fa fa-times-circle"></i> Lyric not embeded');

    // 播放器重载前的清理
    window.adaminPlayerReload = function () {
    ...
    var adaminMedia = {
        play: function () {
            ...
            if (hasLrc) {
                lrcTime = setInterval(adaminLrc.lrc.play, 500);
                $('#adaminLrc').addClass('show');
        ...
        error: function () {
            clearInterval(cicleTime);
            $player.removeClass('playing');
            adaminTips.show(songSheetList[albumId].songNames[songId] + ' - 资源获取失败！尝试获取下一首...');
            adaminMedia.next();
        },
        seeking: function () {
            adaminTips.show('加载中...')
        ...
        volumechange: function () {
            ...
            adaminTips.show('音量：' + vol + '%');
            $.cookie("adamin_player_volume", audio.volume);
        },
        ...
        next: function () {
            clearInterval(cicleTime);
            random ? adaminMedia.getInfos(window.parseInt(Math.random() * songTotal))
                : adaminMedia.getInfos(adaminMedia.getSongId(songId + 1));
        },
        prev: function () {
            clearInterval(cicleTime);
            random ? adaminMedia.getInfos(window.parseInt(Math.random() * songTotal))
    ...
    var adaminTipsTime = null;
    var adaminTips = {
        show: function (cont) {
            clearTimeout(adaminTipsTime);
            $('#adaminTips').text(cont).addClass('show');
            this.hide()
        },
        hide: function () {
            adaminTipsTime = setTimeout(function () {
                $('#adaminTips').removeClass('show');
            }, 3000)
        }
    };
    //给audio添加监听事件  执行相应的函数
    audio.addEventListener('play', adaminMedia.play, false);
    audio.addEventListener('pause', adaminMedia.pause, false);
    audio.addEventListener('ended', adaminMedia.next, false);
    audio.addEventListener('playing', adaminMedia.playing, false);
    audio.addEventListener('volumechange', adaminMedia.volumechange, false);
    audio.addEventListener('error', adaminMedia.error, false);
    audio.addEventListener('seeking', adaminMedia.seeking, false);
    ...
    $('.pause', $player).click(function () {
        ...
        adaminTips.show('暂停播放 - ' + songSheetList[albumId].songNames[songId]);
        $cover.removeClass('coverplay');
        audio.pause();
        setTimeout(function () {
            adaminTips.show("播放器下次访问将自动暂停");
        }, 4000);
    ...
    $('.switch-ksclrc').click(function () {
        ...
        $('#adaminLrc').toggleClass('hide');
        $('#adaminKsc').toggleClass('hidePlayer');
        if (!$('#adaminLrc').hasClass('hide')) {
            ycgeci = true;
            if (hasLrc) {
                $songFrom3.html('<i class="fa fa-check-circle"></i> 歌词开启')
            }
            adaminTips.show('开启歌词显示');
            songFrom33 = '开启';
        ...
        creat: {
            album: function () {
                ...
                $("li", $albumList).click(function () {
                    var a = $(this).index();
                    $(this).hasClass(cur) ? adaminPlayer.playList.creat.song(a, true)
                        : adaminPlayer.playList.creat.song(a, false);
                    $player.addClass("showSongList")
                });
                songTotal = songSheetList[albumId].songIds.length;

                random ? adaminMedia.getInfos(window.parseInt(Math.random() * songTotal))
                    : adaminMedia.getInfos(adaminMedia.getSongId(0));
        ...
        load: function () {
            ...
            setTimeout(function () {
                ...
                $.ajax({
                    ...
                    url:  "https://parser.wxqun.me/api/music/musicLyric?id=" + songSheetList[albumId].songIds[songId] + "&type="
                        + songSheetList[albumId].songTypes[songId],
                    type: 'GET',
                    dataType: 'script',
                    success: function () {
        ...
        lrc: {
            format: function (cont) {
                ...
                for (var i = 1; i < lrcCont.length; i += 2) {
                    ...
                    if (i == 1) {
                        lrcLine += '<li class="adaminLrc' + timer + ' current" style="color:rgba(' + mainColor + ',1)">' + lrcCont[i + 1] + '</li>'
                    } else {
                        lrcLine += '<li class="adaminLrc' + timer + '">' + lrcCont[i + 1] + '</li>'
                    }
                }
                $('#adaminLrc').html('<ul>' + lrcLine + '</ul>');
                setTimeout(function () {
                        if (audio.paused) {
                            $('.switch-ksclrc').hide();
                            $('.switch-down').css('right', '35px');
    ...
    //获取歌单列表数据
    $.ajax({
        url:  'https://parser.wxqun.me/api/music/musicDoc',
        type: 'GET',
        dataType: 'script',
        success: function () {
            if(playerWidth !== -1){
            ...
            if ($.cookie("adamin_player_volume") == '0.666') {
                volume = (defaultVolume / 100);
                audio.volume = volume;
            }

            // 防止百分百音量无触发事件
            adaminMedia.volumechange();

            albumId = defaultAlbum - 1;

            if (showLrc == 0) {
                //隐藏歌词
                $('#adaminLrc').addClass('hide');
            ...
            if (showGreeting == 1) {
                adaminTips.show(greeting);
            }
            setTimeout(function (args) {
                adaminPlayer.playList.creat.album()
            }, 500)
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            adaminTips.show('歌曲列表获取失败!')
        }
    });
    ...
    function netmusic() {
        audio.src = "https://parser.wxqun.me/api/music/musicUrl?id=" + songSheetList[albumId].songIds[songId]+"&type="+songSheetList[albumId].songTypes[songId];
        ...
        coverImg.onload = function () {
            ...
            $.ajax({
                url:  'https://parser.wxqun.me/api/music/mainColor',
                type: 'GET',
                dataType: 'script',
                data: {
                    url: coverImg.src
        ...
        coverImg.error = function () {
            ...
            setTimeout(function () {
                    adaminTips.show(songSheetList[albumId].songNames[songId] + ' - 专辑图片获取失败！')
                },
                4000)
        };
        $cover.html(coverImg);
    ...
    function startPlay() {
        adaminTips.show('开始从' + songFrom55 + '播放 - ' + songSheetList[albumId].songNames[songId]);
        audio.play();
    }
    ...
    function musicTooltip() {
        if (isPhone){return;}
        $('#adaminPlayer span,#adaminPlayer i').each(function () {
            $('#tooltip').remove();
            if (this.title) {
                var a = this.title;
